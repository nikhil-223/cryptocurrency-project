...
const CurrencyDrop = () => {
  ...
  return (
		<div className="flex justify-between gap-2 px-2 py-2 m-1 my-2 rounded-lg bg-dropdownBoxDark text-white">
			<input type="text" className="drop-input w-4/5 p-2 bg-transparent focus:outline-none " value={dropdownName} onChange={handleChange}/>
			<span className="w-1/5 flex justify-center items-center text-5xl" onClick={showList}>
				<IoMdArrowDropdown/>
			</span>
			<div id='currencyDroplist' className="droplist absolute translate-y-12 bg-dropdownListDark w-28 h-48 flex-col rounded overflow-scroll" style={{display:'none'}}>
				{arr.map((item) => {
